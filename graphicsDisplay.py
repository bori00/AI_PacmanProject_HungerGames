...
# Most code by Dan Klein and John Denero written or rewritten for
# cs188, UC Berkeley.
# Some code from a Pacman implementation by LiveWires, and used /
...
GHOST_SHAPE = [(0, 0.3), (0.25, 0.75), (0.5, 0.3), (0.75, 0.75),
...
class InfoPane:
    def __init__(self, layout, gridSize, energyLevel):
        ...
        self.fontSize = int(
            (layout.width + layout.height) * 25 * 0.02)
        self.textColor = PACMAN_COLOR
        self.drawPane(energyLevel)

    def toScreen(self, pos, y=None):
        """
          Translates a point relative from the bottom left of the
          info pane.
        """
        if y == None:
            x, y = pos
    ...
    def drawEnergyLevelIndicator(self):
        ...
        self.energyLevelBars.append(
            rectangle(self.toScreen(startPosX, startPosY), width,
                      height, ENERGY_BAR_OUTLINE_COLOR,
                      HIGH_ENERGY_COLOR))
        self.energyLevelBars.append(rectangle(
            self.toScreen(startPosX + 2 * width + space, startPosY),
            width, height, ENERGY_BAR_OUTLINE_COLOR,
            HIGH_ENERGY_COLOR))
        self.energyLevelBars.append(rectangle(
            self.toScreen(startPosX + 4 * width + 2 * space,
                          startPosY), width, height,
            ENERGY_BAR_OUTLINE_COLOR, HIGH_ENERGY_COLOR))
        self.energyLevelBars.append(rectangle(
            self.toScreen(startPosX + 6 * width + 3 * space,
                          startPosY), width, height,
            ENERGY_BAR_OUTLINE_COLOR, HIGH_ENERGY_COLOR))
    ...
    def updateEnergyLevel(self, energyLevel, maxEnergyLevel):
        ...
        if energyLevel >= maxEnergyLevel * 0.75:
            ...
            changeColor(self.energyLevelBars[0],
                        CRITICAL_ENERGY_COLOR)
...
class PacmanGraphics:
        ...
        for x in range(walls.width):
            ...
            for y in range(walls.height):
                ...
                block = square((screen_x, screen_y),
                               0.5 * self.gridSize,
                               color=BACKGROUND_COLOR, filled=1,
                               behind=2)
                distx.append(block)
    ...
    def swapImages(self, agentIndex, newState):
        """
          Changes an image from a ghost to a pacman or vis versa (
          for capture)
        """
    ...
    def update(self, newState):
        ...
        if self.agentImages[agentIndex][
            0].isPacman != agentState.isPacman: self.swapImages(
        ...
        self.infoPane.updateEnergyLevel(newState.pacmanEnergyLevel,
                                        newState.initialEnergyLevel)
        if 'ghostDistances' in dir(newState):
            self.infoPane.updateGhostDistances(
                newState.ghostDistances)
    ...
    def drawGhost(self, ghost, agentIndex):
        ...
        rightEye = circle((screen_x + self.gridSize * GHOST_SIZE * (
                    0.3 + dx / 1.5),
                          ...
                          self.gridSize * GHOST_SIZE * 0.2, WHITE,
                          WHITE)
        leftPupil = circle((screen_x + self.gridSize * GHOST_SIZE * (
                    -0.3 + dx),
                            screen_y - self.gridSize * GHOST_SIZE * (
                                        0.3 - dy)),
                           self.gridSize * GHOST_SIZE * 0.08, BLACK,
                           BLACK)
        rightPupil = circle((screen_x + self.gridSize * GHOST_SIZE * (
                    0.3 + dx),
                             screen_y - self.gridSize * GHOST_SIZE * (
                                         0.3 - dy)),
                            self.gridSize * GHOST_SIZE * 0.08, BLACK,
    ...
    def moveGhost(self, ghost, ghostIndex, prevGhost,
                  ghostImageParts):
    ...
    def drawWalls(self, wallMatrix):
        ...
        for xNum, x in enumerate(wallMatrix):
            if self.capture and (
                    xNum * 2) < wallMatrix.width: wallColor = \
            TEAM_COLORS[0]
            if self.capture and (
                    xNum * 2) >= wallMatrix.width: wallColor = \
            TEAM_COLORS[1]
            ...
            for yNum, cell in enumerate(x):
                if cell:  # There's a wall here
                    ...
                    # draw each quadrant of the square based on
                    # adjacent walls
                    wIsWall = self.isWall(xNum - 1, yNum, wallMatrix)
                    eIsWall = self.isWall(xNum + 1, yNum, wallMatrix)
                    nIsWall = self.isWall(xNum, yNum + 1, wallMatrix)
                    sIsWall = self.isWall(xNum, yNum - 1, wallMatrix)
                    nwIsWall = self.isWall(xNum - 1, yNum + 1,
                                           wallMatrix)
                    swIsWall = self.isWall(xNum - 1, yNum - 1,
                                           wallMatrix)
                    neIsWall = self.isWall(xNum + 1, yNum + 1,
                                           wallMatrix)
                    seIsWall = self.isWall(xNum + 1, yNum - 1,
                                           wallMatrix)
    ...
    def drawCapsules(self, capsules):
        ...
        for capsule in capsules:
            ...
            dot = circle((screen_x, screen_y),
                         ...
                         fillColor=CAPSULE_COLOR, width=1)
    ...
    def drawExpandedCells(self, cells):
        ...
        for k, cell in enumerate(cells):
            ...
            block = square(screenPos, 0.5 * self.gridSize,
    ...
    def clearExpandedCells(self):
        if 'expandedCells' in dir(self) and len(
        ...
        for x in range(len(self.distributionImages)):
            for y in range(len(self.distributionImages[0])):
                ...
                for weight, gcolor in zip(weights, colors):
                    color = [min(1.0, c + 0.95 * g * weight ** .3) for
                             c, g in zip(color, gcolor)]
...
def saveFrame():
    ...
    name = os.path.join(POSTSCRIPT_OUTPUT_DIR,
                        'frame_%08d.ps' % FRAME_NUMBER)
    FRAME_NUMBER += 1
